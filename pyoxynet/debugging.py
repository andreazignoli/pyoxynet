...
for i_ in np.arange(100):
    ...
    if plotting:
        ...
        plt.vlines(x=int(data_dict_fake['info']['VT1']), ymin=0, ymax=100)
        plt.vlines(x=int(data_dict_fake['info']['VT2']), ymin=0, ymax=100)
        plt.subplot(2, 2, 3)
        plt.plot(df.time, df.VEVO2_I)
        plt.plot(df.time, df.VEVCO2_I)
        plt.vlines(x=int(data_dict_fake['info']['VT1']), ymin=0, ymax=0.05)
        plt.vlines(x=int(data_dict_fake['info']['VT2']), ymin=0, ymax=0.05)
        plt.subplot(2, 2, 4)
        plt.plot(df.time, df.PetO2_I, 'o')
        plt.vlines(x=int(data_dict_fake['info']['VT1']), ymin=100, ymax=120)
        plt.vlines(x=int(data_dict_fake['info']['VT2']), ymin=100, ymax=120)
        here=0
        plt.cla()
        ...
        plt.vlines(x=int(data_dict_fake['info']['VT1']), ymin=-1, ymax=1)
        plt.vlines(x=int(data_dict_fake['info']['VT2']), ymin=-1, ymax=1)
