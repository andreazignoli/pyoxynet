...
def create_probabilities(duration=600, VT1=320, VT2=460, training=True, normalization=False, resting = True, resting_duration=60, initial_step=False, y_pm0=1):
    ...
    if not initial_step:
        ...
        # moderate => 2 splines
        pm_L1 = UnivariateSpline([0, 60, 120, VT1], [y_pm0, (y_pm0 - 0.5)/2 + 0.5, (y_pm0 - 0.5)/2 + 0.5, 0.5], k=3)
        # heavy => 3 splines
        ph_L1 = UnivariateSpline([0, 60, 120, VT1], [1 - y_pm0, 1 - (y_pm0 - 0.5)/2 + 0.5, 1 - (y_pm0 - 0.5)/2 + 0.5, 0.5], k=3)
